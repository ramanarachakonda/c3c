...
def get_max_cases(hist_df, city, current):
    ...
    if not city_df.empty:
        prev_max = max(city_df['Total_Cases'])
        county = city_df['COUNTY'].values[0]
        state = city_df['State'].values[0]
        if (current < prev_max): print("*** SOMETHING IS WRONG", city, 'current=',current,'PrevMax=',prev_max)
        return state, county, current, not(current < prev_max)
    else:
        print("*** CITY NOT FOUND ", city, 'current=',current)
        return None, city, current, True
...
def main():
    ...
    for key, t in dic.items():
        ...
        for city in parsed_json['features']:
            ...
            if cityname == 'WASHINGTON TWP': continue
    ...
    for m in re.findall(r';([A-Z\s*]+\s*(TOWNSHIP|BOROUGH|CITY|HI-NELLA|TAVISTOCK))\s*<br>\s*([0-9]+);([0-9]+)', html_content, re.MULTILINE | re.DOTALL):
        ...
        state, county, new_current, changed = get_max_cases(hist_df, city, int(total_cases))
        if not state:
            state = 'New Jersey'
            county = 'Camden'
